...
Const scrwid=1024
Const scrhgh=768
Const depth=16           ;This may work better at 32
...
Function Render()
	For p.sprk=Each sprk
		If p\x>=0 And p\y>=0 And p\x<=scrwid-1 And p\y<=scrhgh-1 Then WritePixelFast p\x,p\y,CompressInts(p\b*p\br,p\g*p\br,p\r*p\br,255)
;		Color p\r*p\br,p\g*p\br,p\b*p\br
;		Plot p\x,p\y
	Next
	For f.frag=Each frag
		If f\br>fadespeed*15
			If f\x>=0 And f\y>=0 And f\x<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x,f\y,CompressInts(255,255,255,255)
			If f\x-1>=0 And f\y>=0 And f\x-1<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x-1,f\y,CompressInts(f\b,f\g,f\r,255)
			If f\x+1>=0 And f\y>=0 And f\x+1<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x+1,f\y,CompressInts(f\b,f\g,f\r,255)
			If f\x>=0 And f\y-1>=0 And f\x<=scrwid-1 And f\y-1<=scrhgh-1 Then WritePixelFast f\x,f\y-1,CompressInts(f\b,f\g,f\r,255)
			If f\x>=0 And f\y+1>=0 And f\x<=scrwid-1 And f\y+1<=scrhgh-1 Then WritePixelFast f\x,f\y+1,CompressInts(f\b,f\g,f\r,255)
;			Color 255,255,255
;			Plot f\x,f\y
...
;			Plot f\x,f\y+1
		Else
			If f\x>=0 And f\y>=0 And f\x<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x,f\y,CompressInts(f\b*br*4,f\g*f\br*4,f\r*f\br*4,255)
			If f\x-1>=0 And f\y>=0 And f\x-1<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x-1,f\y,CompressInts(f\b*f\br,f\g*f\br,f\r*f\br,255)
			If f\x+1>=0 And f\y>=0 And f\x+1<=scrwid-1 And f\y<=scrhgh-1 Then WritePixelFast f\x+1,f\y,CompressInts(f\b*f\br,f\g*f\br,f\r*f\br,255)
			If f\x>=0 And f\y-1>=0 And f\x<=scrwid-1 And f\y-1<=scrhgh-1 Then WritePixelFast f\x,f\y-1,CompressInts(f\b*f\br,f\g*f\br,f\r*f\br,255)
			If f\x>=0 And f\y+1>=0 And f\x<=scrwid-1 And f\y+1<=scrhgh-1 Then WritePixelFast f\x,f\y+1,CompressInts(f\b*f\br,f\g*f\br,f\r*f\br,255)
...
;			Plot f\x,f\y+1
	...
	Next
	For s.shot=Each shot
		If s\x>=0 And s\y>=0 And s\x<=scrwid-1 And s\y<=scrhgh-1 Then WritePixelFast s\x,s\y,CompressInts(255,255,255,255)
		If s\x+1>=0 And s\y>=0 And s\x+1<=scrwid-1 And s\y<=scrhgh-1 Then WritePixelFast s\x+1,s\y,CompressInts(0,0,255,255)
		If s\x-1>=0 And s\y>=0 And s\x-1<=scrwid-1 And s\y<=scrhgh-1 Then WritePixelFast s\x-1,s\y,CompressInts(0,0,255,255)
		If s\x>=0 And s\y+1>=0 And s\x<=scrwid-1 And s\y+1<=scrhgh-1 Then WritePixelFast s\x,s\y+1,CompressInts(0,0,255,255)
		If s\x>=0 And s\y-1>=0 And s\x<=scrwid-1 And s\y-1<=scrhgh-1 Then WritePixelFast s\x,s\y-1,CompressInts(0,0,255,255)
;		Color 255,255,255
;		Plot s\x,s\y
;		Color 255,0,0
