...
'Der Quelltext ist frei , jedoch m?chte ich nicht das daraus
...
Type TZone
 ...
 Field wi:Int 'Winkel f?r blinkende Farbe
...
Function MainLoop()
 ...
 '4.Texture Gr??e w?hlen
 '5.Bild umrechnen
 '6.Bild anzeigen als Tile
 '7.Bild anzeigen als Tile zum ?berblenden
 '8.Bild speichern
 ...
 Local pix:TPixmap
 Local pix2:TImage
 Local pix3:TImage 'f?r Blend

 Local filename:String
 Local filenamesave:String

 Local p:TV3D=TV3D.Create() 'f?r Plot der Bezier Splines
 ...
 Local mu:Float=0 'f?r Bezier4 Splines
 ...
 Local t1 'Timer f?r konstante Frame Rate
 ...
 While Not KeyHit(KEY_ESCAPE)
  ...
  Case mode_LoadImageDialog
   ...
   DebugLog "Bild ausw?hlen in "+AppDir 

   filename=RequestFile("Please select a image","Image jpg,jpeg,pcx,tga,bmp,gif,png:jpg,jpeg,pcx,tga,bmp,gif,png;All Files *.*:*",False) ',AppDir+"\") schei?e
   If Len(filename)=0 Then
    DebugLog "Kein Bild ausgew?hlt , ENDE"
    End
   Else
    mode=mode_LoadImage
   EndIf
  ...
  Case mode_LoadImage
   ...
   DebugLog "ab jetzt Punkte w?hlen"
  ...
  Case mode_SelectPoints
   ...
   'Bei DrawPixMap , eine Pixmap mu? ins Fenster passen sonnst kommt ein Fehler !!!
   ...
   EndIf
   'bewegen mit gedr?ckter Maustaste
  ...
  Case mode_ShowBlend
   ...
   DrawText "Blending",32*10+4*10+10,16-TextHeight("B")/2
    ...
    'XY ?berg?nge berechnen (2 Pass sonnst ?berkreuzt sich das)
  ...
  Case mode_SaveAsDialog
   ...
   If Len(filenamesave)=0 Then
    DebugLog "Kein Dateiname zum speichern ausgew?hlt"
  ...
  If KeyHit(KEY_SPACE) Or (md1=1 And Zone=cZoneWeiter) Then 'Weiter zum n?chsten Schritt
   Select mode
  ...
  If KeyHit(KEY_BACKSPACE) Or (md1=1 And Zone=cZoneZurueck) Then 'Zur?ck zum letzten Schritt
   Select mode
...
Function TransformImage:TImage(pix:TPixmap,txx:Float,txy:Float,p1:TV3D,p2:TV3D,p3:TV3D,p4:TV3D,p1a:TV3D,p1b:TV3D,p2a:TV3D,p2b:TV3D,p3a:TV3D,p3b:TV3D,p4a:TV3D,p4b:TV3D)
 ...
 Local mu:Float=0,p:TV3D=TV3D.Create(),db:Int=0 'F?r au?en Curven
 ...
 Local Rechts:TV3D=TV3D.Create()
	...
	For y=0 To txy
     ...
     If (x Mod 8)=0 Or (y Mod 8)=0 Then
	  ...
	  SetColor 255,zz,zz 'welcher Bereich ausgelesen wird
	  Plot xx,yy
	 EndIf
	 '<<<
	Next
 ...
 For Zone=EachIn Zonen
  ...
  If Zone.Mode=ZoneMode Or Zone.Mode=0 Then
   ...
   If Zone.Visible=1 Then   
    ...
    'Wenn Maus dr?ber dann Hintergrund f?llen  
    If Hit=True Then
    ...
    'Wenn Maus dr?ber dann Rand zeigen in gr?n
    SetAlpha 1
...
Function BlendImage:TImage(img:TImage,BlendX,BlendY,BlendRange:Double)
 ...
 Local RangeX:Int=0 'Rand Bereich au?en in Pixel
 Local RangeY:Int=0
 ...
 For x=0 To PixmapWidth(map)-1
  For y=0 To PixmapHeight(map)-1
   ...
   Else
    'ARGB=0 'Test um den unber?hrten Bereich zu sehen
   EndIf
...
Function VNORMAL(p1:TV3D,p2:TV3D,p3:TV3D,n:TV3D Var)
 ...
 'Oberfl?chen Normale von einer Ebene mit 3 Punkten (Dreieck)
...
Function VNORMALIZE(a:TV3D Var)
 ...
 'gibt Normvector zur?ck , aufpassen auf ?berlauf !

 'also gesamt Vector auf l?nge 1 bringen 
...
Function VTRANS(a:TV3D Var)
 ...
 '?berschribt den Vector ! AUFPASSEN ! 
