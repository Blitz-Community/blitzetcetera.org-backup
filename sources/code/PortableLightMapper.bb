...
;CreatelmLight(x,y,z,red,green,blue,range)
...
createlmlight(0,0,0,0,0,255,15) ;Position the lights
createlmlight(-5,0,0,255,0,0,20) ;light 2 is at -5,0,0 and color is 255,0,0 and range is 20
While Not KeyHit(1)
If MouseHit(1)
...
SaveBuffer(TextureBuffer(tex),"shared.bmp")
FreeTexture tex
EndIf
EndIf
...
lightmapbound = t\size*t\size
     For ypos = 0 To lightbound-1
          ...
          For nl.lightmapnode = Each lightmapnode
               If RectsOverlap(nl\x,nl\y,nl\width,nl\height,xpos,ypos,t\size,t\size) = 1
               overlap=1:xpos=xpos+nl\width-1
               Exit
               EndIf
          Next
          If xpos + t\size > lightbound Then overlap = 1:dot1=1
          If ypos + t\size > lightbound Then overlap = 1:dot2=1
          If overlap = 0
          t\xpos = xpos : t\ypos = ypos
               n.lightmapnode = New lightmapnode
               n\x = xpos : n\y = ypos : n\width = t\size : n\height = t\size
               ...
               If scaletex = 1
               If t\u[co] = 1 Then t\u[co] = t\u[co]-.1
               If t\v[co] = 1 Then t\v[co] = t\v[co]-.1
               If t\u[co] = 0 Then t\u[co] = t\u[co]+.1
               If t\v[co] = 0 Then t\v[co] = t\v[co]+.1
               EndIf
...
Function CreateLMLight(x#,y#,z#,r#,g#,b#,range#)
